...
Couch.prototype.get = function (id, cb) {
  request({url:this.url+id, json:true}, function (err, resp, doc) {
    ...
    if (resp.statusCode !== 200) {
      var e = doc ? doc : new Error('CouchDB get error.') // make this smarter later
...
Couch.prototype.getCached = function (id, cb) {
  var self = this
  if (!self.cache) self.cache = {}
  if (self.cache[id]) {
    cb(false, self.cache[id])
  } else {
...
View.prototype.query = function (opts, cb) {
  ...
  r(function (e, resp, body) {
    if (e) return cb(e)
    if (resp.statusCode !== 200) {
      var e = body ? body : new Error('CouchDB query error.') // make this smarter later
      e.statusCode = resp.statusCode
      return cb(e)
    }
    cb(null, body)
...
module.exports.create = function (url, name, cb) {
  ...
  jsonreq.put(url, function (e, resp, body) {
    if (e) return cb(e)
    if (resp.statusCode !== 201) {
      var e = body ? body : new Error('CouchDB create error.') // make this smarter later
      e.statusCode = resp.statusCode
      return cb(e)
    }
    cb(null, body)
