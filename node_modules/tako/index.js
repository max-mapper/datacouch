...
if (process.logging) var log = process.logging('tako')
...
function Application (options) {
  ...
  self.handler = function (req, resp) {
    ...
    resp.error = function (err) {
      ...
      if (log) log('error %statusCode "%message "', e)
...
Application.prototype.listen = function (createServer, port, cb) {
  ...
  self.server = createServer(function (req, resp) {
    console.log(JSON.stringify(new Date()), req.url)
      ...
      if (authHandler) {
        ...
        authHandler(req, resp, function (user) {
          ...
          if (self._must && self._must.indexOf('auth') !== -1 && !req.user) {
            resp.statusCode = 403
            resp.end('This resource requires auth.')
            return
          }
          self.emit('request', req, resp)
          req.release()
